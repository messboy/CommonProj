...
namespace Backend.Security
{
    public class CustomMembershipProvider : MembershipProvider
    {
        ...
        public override bool ValidateUser(string username, string password)
        {
            ...
            AdminService adminManagement = new AdminService();

            List<AdminInfo> adminList = adminManagement.GetAll();
        ...
        public override MembershipUser GetUser(string username, bool userIsOnline)
        {
            // 取得權限資訊
            AdminService adminManagement = new AdminService();
            var account = adminManagement.GetAuth(username);

            var cacheKey = string.Format("UserData_{0}", username);
            if (HttpRuntime.Cache[cacheKey] != null )
                //&& // 驗證館是否改變 (TODO 其他應該也需要驗證)
                //((CustomMembershipUser)HttpRuntime.Cache[cacheKey]).Museum == account.Museum)
            {
